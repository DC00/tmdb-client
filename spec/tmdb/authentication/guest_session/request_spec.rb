describe Tmdb::Authentication::GuestSession::Request do
  ...
  describe "#execute" do
    ...
    context "when happy" do
      ...
      it "has correct url" do
        expect(adapter).to receive(:execute).with(:get, GUEST_SESSION_URL, {})
    ...
    context "when mock_pass" do
      ...
      it "returns mock pass response" do
        ...
        expect(response.success?).to eq(true)
        expect(response.success).to eq(true)
        expect(response.guest_session_id).to be_present
